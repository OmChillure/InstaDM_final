...
// Modify the "Check Authentication" button click handler
    ...
    chrome.runtime.sendMessage({ action: "checkAuth" }, (response) => {
      if (chrome.runtime.lastError) {
        ...
        if (response.isLoggedIn) {
          alert(`User is logged in as jay`);
          console.log('Username:', response);
        const user = window.localStorage.setItem('instagramUsername', response.username);
        const getuser = window.localStorage.getItem('instagramUsername');
        console.log(getuser)
        } else {
