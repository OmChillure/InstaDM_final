...
const DashboardWrapper = <P extends object>(WrappedComponent: ComponentType<P>, head:string,  className?:string): FC<P> => {
    const Wrapper: FC<P> = (props) => {
        return (
            <div className={clsx("w-full flex-1 py-9 px-8 box-border flex flex-col gap-4 overflow-y-auto",className)}>
                {/* <span className={clsx(typoGraphy.text48normal,"text-lightgray02 dark:text-gray02")}>
                    {breadCrumbs?.join("  >  ")}
                </span> */}
